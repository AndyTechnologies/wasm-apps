...
static int define_exports(Linker &linker, Store::Context ctx, Instance instance, const char* instance_label) {
  ...
  if (std::strcmp(instance_label, "instance0") == 0) {
    if (_defined.find("compute") == _defined.end()) {
      _defined.insert("compute");
      auto exp = instance.get(ctx, "compute");
      if (!exp) { std::cerr << "Error obteniendo export compute" << std::endl; return 1; }
      auto result = linker.define(ctx, "env", "compute", *exp);
      if (!result) { std::cerr << "Error definiendo compute: " << result.err().message() << std::endl; return 1; }
    }
    return 0;
  }
...
int main(int argc, char *argv[]) {
  ...
  auto entry_exp = instance0.get(ctx, "compute");
  if (!entry_exp) { std::cerr << "Entry point compute no encontrado" << std::endl; return 1; }
  if (!std::get_if<Func>(&*entry_exp)) { std::cerr << "compute no es una funcion" << std::endl; return 1; }
  auto entry_func = std::get<Func>(*entry_exp);
  auto result = entry_func.call(ctx, {});
  if (!result) {
    std::cerr << "Error llamando a compute" << std::endl;
    return 1;
  }
