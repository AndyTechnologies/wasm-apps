...
const KNOWN_IMPLS: Record<string, EnvImpl> = {
  ...
  'Math.clz32': { needsMemory: false, hasReturn: true, body: `uint32_t _v = (uint32_t)args[0].f64(); results[0] = Val(double(_v == 0 ? 32 : __builtin_clz(_v))); return std::monostate{};` },
...
export function generateCCode(
  ...
  let cpp = [
    ...
    '  size_t mlen = strnlen(reinterpret_cast<const char*>(data + ptr), sz - ptr);',
